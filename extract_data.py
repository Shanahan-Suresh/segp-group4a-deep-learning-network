...
def get_chemicals(initial_row,initial_column):
    ...
    while (not blank_row):
        ...
        global finaldf
        finaldf = pd.concat([finaldf,bigdata],ignore_index=True) #concatanates the temporary dataframe into the final dataframe
        del bigdata
...
def findParameterData():
    ...
    while initial_row != max_row:
        ...
        if initial_row == max_row:
            ...
            #exit loop at final coloumn, else row increment by 2
...
def modifyDataset():
    ...
    finaldf.loc[finaldf['Chemicals'].str.contains("Lauric acid",na=False) & finaldf['Chemicals'].str.contains("Stearic acid",na=False), 'Chemicals'] = 4
